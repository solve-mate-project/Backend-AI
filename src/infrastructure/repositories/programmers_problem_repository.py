...
class ProgrammersProblemRepository(ProblemRepository):
    async def get_problem_by_url(self, url: str) -> Optional[Problem]:
        """
        프로그래머스 URL을 받아 문제 정보를 파싱하여 Problem 엔티티로 반환합니다.
        """
        # URL 유효성 검사
        if not re.match(
        ...
        try:
            ...
            # 문제 제목 추출
            title_element = soup.select_one(".challenge-title")
            title = (
            ...
            # 문제 내용 추출
            content_element = soup.select_one(".guide-section")
            problem_content = ""
            if content_element:
